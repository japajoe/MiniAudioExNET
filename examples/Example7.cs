...
namespace MiniAudioExExamples
{
	class Example7
	{
		...
		static unsafe void Main(string[] args)
		{
			...
			if (MiniAudioNative.ma_context_init(null, pContext) != ma_result.MA_SUCCESS)
			{
			...
			ma_device_config deviceConfig = MiniAudioNative.ma_device_config_init(ma_device_type.ma_device_type_playback);
			...
			if (MiniAudioNative.ma_context_get_devices(pContext, out ma_device_info_ex[] ppPlaybackDeviceInfos, out ma_device_info_ex[] ppCaptureDeviceInfos) != ma_result.MA_SUCCESS)
			{
			...
			if (MiniAudioNative.ma_device_init(pContext, ref deviceConfig, pDevice) != ma_result.MA_SUCCESS)
			{
			...
			ma_decoding_backend_vtable** pCustomBackendVTables = stackalloc ma_decoding_backend_vtable*[1];
			pCustomBackendVTables[0] = MiniAudioNative.ma_libvorbis_get_decoding_backend();

			ma_resource_manager_config resourceManagerConfig = MiniAudioNative.ma_resource_manager_config_init();
			resourceManagerConfig.ppCustomDecodingBackendVTables = (IntPtr)pCustomBackendVTables;
			resourceManagerConfig.customDecodingBackendCount = 1;

			if (MiniAudioNative.ma_resource_manager_init(ref resourceManagerConfig, pResourceManager) != ma_result.MA_SUCCESS)
			{
			...
			if (MiniAudioNative.ma_engine_init(ref engineConfig, pEngine) != ma_result.MA_SUCCESS)
			{
			...
			if (MiniAudioNative.ma_device_start(pDevice) != ma_result.MA_SUCCESS)
			{
			...
			if (MiniAudioNative.ma_sound_init_from_file(pEngine, "some_audio.ogg", ma_sound_flags.MA_SOUND_FLAG_STREAM, default, default, pSound) != ma_result.MA_SUCCESS)
			{
